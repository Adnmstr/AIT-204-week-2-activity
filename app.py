...
Predicts exam scores based on daily study hours using Linear Regression (Gradient Descent).
"""
...
heteroscedastic = st.sidebar.checkbox(
    "Heteroscedastic noise (variance depends on study hours)",
    value=False,
    help="If checked, noise increases with study hours and the model trains with weights."
)
...
# NOTE: generate_data now returns (study_hours, exam_scores, sample_weights)
study_hours, exam_scores, sample_weights = data_gen.generate_data(
...
if heteroscedastic:
    st.info("Heteroscedastic mode ON: noise variance changes with study hours; the model trains with sample weights.")
...
with st.spinner("Training model..."):
    # pass weights into fit so the model itself reflects the checkbox
    model.fit(study_hours, exam_scores, sample_weights=sample_weights)

params = model.get_parameters()
metrics = model.calculate_metrics(study_hours, exam_scores, sample_weights=sample_weights)
...
with col1:
    st.metric("Slope (m)", f"{params['slope']:.3f}", help="Points gained per additional study hour")
with col2:
    st.metric("Intercept (b)", f"{params['intercept']:.2f}", help="Expected score with zero study hours")
...
fig_fit.add_trace(go.Scatter(
    ...
    mode="markers",
    name="Predictions",
    marker=dict(size=6, opacity=0.8, symbol="x")
))
...
with col1:
    ...
    st.markdown(f"**Final Cost**: {history['cost'][-1]:.2f}")
...
with col2:
    ...
    st.markdown("**Residuals** should be randomly scattered around zero. Patterns indicate model limitations.")
...
fig_params.add_trace(
    go.Scatter(x=history["iteration"], y=history["slope"], mode="lines", name="Slope"),
    row=1, col=1
)
fig_params.add_trace(
    go.Scatter(x=history["iteration"], y=history["intercept"], mode="lines", name="Intercept"),
    row=1, col=2
)
...
st.markdown("Watch how parameters start from zero and converge to optimal values. The curves flatten when gradients approach zero.")
...
st.markdown("Use the trained model to predict exam scores for different study hour values.")
...
with col2:
    ...
    st.markdown(
        f"**Prediction**: With **{study_input:.1f} hours** of daily study, "
...
if heteroscedastic:
    # Variable-width uncertainty band based on the generator's scale function
    scale = data_gen.heteroscedastic_scale(study_range)
    ref_scale = np.mean(data_gen.heteroscedastic_scale(study_hours))
    local_sigma = metrics["rmse"] * (scale / ref_scale)

    upper = predicted_range + local_sigma
    lower = predicted_range - local_sigma

    band_name = "Uncertainty (heteroscedastic)"
else:
    upper = predicted_range + metrics["rmse"]
    lower = predicted_range - metrics["rmse"]
    band_name = "Uncertainty (±RMSE)"
...
if heteroscedastic:
    st.markdown(
        f"The shaded region reflects **heteroscedastic uncertainty** (it widens/narrows with study hours). "
        f"Your prediction is marked with a star."
    )
else:
    st.markdown(
        f"The shaded region represents uncertainty (±{metrics['rmse']:.1f} points). "
        f"Your prediction is marked with a star."
    )
